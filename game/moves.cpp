    ...
    for (int rank = 7; rank >= 0; --rank) {
        for (int file = 0; file < 8; ++file) {
            ...
            if (piece_at_position.color == turn) {
                ...
                moves.insert(moves.end(), moves_at_Piece.begin(),
                    moves_at_Piece.end());
            }
        }
    }
    ...
    const BoardPieces& board)
{
    ...
    for (auto position : positions) {
        if (position.IsValidPosition()) {
            Piece piece = Board::GetPieceAtPosition(position, board);
            if (piece.color != captured_piece.color && piece.type != PIECETYPE_NIL) {
                moves.push_back({ .curr = position,
...
Moves GetAllMoves(const BoardState state)
{
    ...
    auto enpassant_captures = GetEnpassantCaptures(state.enpassant_target, state.board);
    moves.insert(moves.end(), enpassant_captures.begin(),
        enpassant_captures.end());

    auto castling_moves = GetCastlingMoves(state.turn, state.castling_rights, state.board);
    ...
    const BoardPieces& board)
{
    ...
    Piece king_piece = GetPieceAtPosition({ .rank = rank, .file = king_file }, board);
